...
sudo tcpdump -ne -c 2000 -w packets.pcap || echo "tcpdump fails" exit 1

# Filter and save ARP, UDP, and TCP packets into separate pcap files
sudo tcpdump -ne -r packets.pcap arp -w arp_packets.pcap
sudo tcpdump -ne -r packets.pcap udp -w udp_packets.pcap
sudo tcpdump -ne -r packets.pcap tcp -w tcp_packets.pcap

# Convert pcap files to text for further processing
sudo tcpdump -ne -r arp_packets.pcap > arp_packets.txt
sudo tcpdump -ne -r udp_packets.pcap > udp_packets.txt
sudo tcpdump -ne -r tcp_packets.pcap > tcp_packets.txt
